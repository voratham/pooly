defmodule Pooly.PoolServer do
  ...
  def handle_info(
        ...
        state = %{name: name, pool_sup: pool_sup, mfa: mfa, size: size}
      ) do
  ...
  def handle_info(
        {:EXIT, pid, _reason},
        state = %{monitors: monitors, workers: workers, pool_sub: pool_sub}
      ) do
    case :ets.lookup(monitors, pid) do
      [{pid, ref}] ->
        true = Process.demonitor(ref)
        true = :ets.delete(monitors, pid)
        {:noreply, %{state | workers: [new_worker(pool_sub) | workers]}}
